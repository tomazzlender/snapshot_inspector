  ...
  module Snapshot
    class Snapshot
      ...
      private_class_method def self.all
        file_paths = Helpers.persisted_snapshots_file_paths(Minitest::Snapshot.configuration.storage_directory)
      ...
      module Helpers
        def self.absolute_snapshot_file_path(slug)
          Minitest::Snapshot.configuration.storage_directory.join("#{slug}.json")
        end
        ...
        def self.extract_slug(file_path)
          file_path.gsub(Minitest::Snapshot.configuration.storage_directory.to_s + "/", "").gsub(".json", "")
        end
