module ViewInspector
  class Storage
    class << self
      def write(key, value)
        file_path = to_file_path_for_writing(key)
        file_path.dirname.mkpath
        file_path.write(value)
      end

      def read(key)
        File.read(to_file_path_for_reading(key))
      end

      def list
        Dir
          .glob("#{ViewInspector.configuration.absolute_storage_directory}/**/*.{json}")
          .map { |file_path| to_key(file_path) }
      end

      def clean_storage_directory
        ViewInspector.configuration.absolute_storage_directory.rmtree
      end

      def clean_processing_directory
        ViewInspector.configuration.absolute_processing_directory.rmtree
      end

      def move_processing_to_storage_directory
        ViewInspector.configuration.absolute_storage_directory.rmtree
        ViewInspector.configuration.absolute_processing_directory.rename(ViewInspector.configuration.absolute_storage_directory)
      end

      private

      def to_key(file_path)
        file_path.gsub(ViewInspector.configuration.absolute_storage_directory.to_s + "/", "").gsub(".json", "")
      end

      def to_file_path_for_reading(key)
        ViewInspector.configuration.absolute_storage_directory.join("#{key}.json")
      end

      def to_file_path_for_writing(key)
        ViewInspector.configuration.absolute_processing_directory.join("#{key}.json")
      end
    end
  end
end
